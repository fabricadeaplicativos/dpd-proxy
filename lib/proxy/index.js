...
var initiate = function(options) {
	...
	*/
	...
	app.post('/resources', function(req, res) {
		...
		if (!fs.existsSync(path.join(resourcesDirectory, folderName))) {
			...
			fs.writeFile(path.join(resourcesDirectory, folderName, CONFIG_FILE), JSON.stringify(config, null, 4), function(err) {
				...
				res.set('Access-Control-Allow-Origin', 'http://localhost:3000/client/').status(201).json({apiVersion: API_VERSION, data: {"collectionId": folderName}, "status": "Ok"});
			});
		} else {
			res.set('Access-Control-Allow-Origin', 'http://localhost:3000/client/').status(500).json({
	...
	app.put('/resources', function(req, res) {
		...
		for (oldCollection in req.body.collections) {
			...
			proxyRequestToDeployd(config, options, function(chunk) {
				res.set('Access-Control-Allow-Origin', 'http://localhost:3000/client/').status(201).json({apiVersion: API_VERSION, data: JSON.parse(chunk)});
			});
		}
	});
	...
	app.get('/resources', function(req, res) {
		...
		res.status(200).json({apiVersion: API_VERSION, data: dirs});
	});
	...
	app.put('/resources/:collection', function(req, res) {
		...
		fs.writeFile(p, JSON.stringify(config, null, 4), function(err) {
			...
			res.set('Access-Control-Allow-Origin', 'http://localhost:3000/client/').json({apiVersion: API_VERSION, status: "Ok"});
		});
	});
	...
	app.post('/:collection', function(req, res) {
		...
		var proxyRequest = http.request(options, function(response) {
			...
			response.on('data', function (chunk) {
				...
				res.set('Access-Control-Allow-Origin', 'http://localhost:3000/client/').status(201).json({
					apiVersion: API_VERSION,
		...
		var proxyRequest = http.request(options, function(response) {
			...
			response.on('data', function (chunk) {
				...
				fs.writeFile(p, JSON.stringify(config, null, 4), function(err) {
					...
					res.set('Access-Control-Allow-Origin', 'http://localhost:3000/client/').status(200).json({apiVersion: API_VERSION, "status": "Ok"});
				});			
			});
		});
	...
	app.put('/:collection/:documentId([a-zA-Z0-9]+$)', function(req, res) {
		...
		var proxyRequest = http.request(options, function(response) {
			...
			response.on('data', function (chunk) {
				...
				res.set('Access-Control-Allow-Origin', 'http://localhost:3000/client/').status(200).json({
					apiVersion: API_VERSION,
					data: JSON.parse(chunk)
				});
			});
	...
	app.get('/:collection/config', function(req, res) {
		...
		res.status(200).json({apiVersion: API_VERSION, data: config});
	});
	...
	app.get('/resources/config', function(req, res) {
		...
		// res.status(200).json({apiVersion: API_VERSION, data: arrayOfConfigs});
	});
